...
describe("GitHub PR extension lifecycle", () => {
  test("fetches, delegates to patch, and removes the temporary patch on shutdown", async () => {
    ...
    const result = await handler(["pr", "123", "--", "--pager"], context);
  ...
  test("retains delegated patches while a replacement registry adopts the same factory", async () => {
    ...
    const result = await first.handler(["pr", "1", "--repo", "owner/repo"], {
      cwd: "/repo",
      signal: new AbortController().signal,
      stdin: { async *[Symbol.asyncIterator]() {} },
      stdout: { async write() {} },
  ...
  test("owns extension help without reading Git or the network", async () => {
    ...
    const output: string[] = [];
    const result = await handler(["pr", "--help"], {
      cwd: "/repo",
      signal: new AbortController().signal,
      stdin: { async *[Symbol.asyncIterator]() {} },
      stdout: {
    ...
    expect(output.join("")).toContain("Usage: hunk gh pr");
  ...
  test("declares a runtime-dependency-free API-v10 folder extension", () => {
    ...
    const manifest = JSON.parse(readFileSync(join(testDirectory, "package.json"), "utf8")) as {
